 ...
 * "
 *
 * @NotBlank: 这个注解用于检查字符串是否为非空且不为仅空白字符。它会检查字段是否为 null、长度为0或只有空白字符（如空格、制表符等）。
 * 对于 String 类型来说，这表示字符串必须至少包含一个非空白字符。
 * @NotEmpty: 这个注解比 @NotBlank 更宽松一点，它只检查字段是否为 null 或者集合、数组、Map 是否为空（即长度或大小为0）。
