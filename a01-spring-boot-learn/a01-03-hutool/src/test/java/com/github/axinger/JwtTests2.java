...
public class JwtTests2 {
    @Test

    //JWT头部和载荷是Base64编码，非加密，属于明文；签名确保不被篡改，
    // 但需密钥保密；密钥长度足够时难以破解；建议措施如使用HTTPS、避免敏感数据、密钥轮换等。
    public void test1() {
        String username = "jim";
...
// 或者从固定字符串生成（需确保足够安全）
        String secretString = "my-very-secure-secret-that-is-at-least-256-bits-long2";
        SecretKey key = Keys.hmacShaKeyFor(secretString.getBytes(StandardCharsets.UTF_8));

        String format = key.getFormat();
        System.out.println("format = " + format);
        String string = new String(key.getEncoded());
        System.out.println("key getEncoded = " + string);
        String jwt = Jwts.builder()
                .subject(username)
                .issuedAt(DateUtil.date(signTime))
                .expiration(DateUtil.date(expiresAt))
                .signWith(key)//加密方式,解密密码
//                .signWith(key, SignatureAlgorithm.HS256)//加密方式,解密密码
                .claim("name", "tom2")
                .claim("sex", "男")
                .compact();
        ...
        try {
            ...
            System.out.println(new String(claims.getDigest()));
